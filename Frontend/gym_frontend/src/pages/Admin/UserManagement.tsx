      ...
      <main className="flex-grow p-4 md:p-10 max-w-7xl mx-auto w-full space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-brand-matte tracking-tighter uppercase">Operative <span className="text-brand">Registry</span></h1>
            <p className="text-brand-matte/40 font-bold uppercase tracking-widest text-[10px]">Manage operative clearances and account statuses.</p>
          </div>
          <div className="relative">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-matte/30 w-5 h-5" />
            <input type="text" placeholder="Locate operative profile..." className="pl-16 pr-6 py-5 bg-white border border-brand-matte/10 outline-none focus:border-brand-gold/40 text-[11px] font-black uppercase tracking-widest text-brand-matte w-full md:w-[450px] shadow-2xl" />
          </div>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-brand-warm">
                  <th className="px-10 py-6 text-[10px] font-black text-brand-matte/40 uppercase tracking-[0.3em]">Operative Profile</th>
                  <th className="px-10 py-6 text-[10px] font-black text-brand-matte/40 uppercase tracking-[0.3em]">Clearance</th>
                  <th className="px-10 py-6 text-[10px] font-black text-brand-matte/40 uppercase tracking-[0.3em]">Enlist Date</th>
                  ...
                  <tr key={user.id} className="group hover:bg-brand-warm transition">
                    ...
                    <td className="px-10 py-8">
                      <span className={`inline-block px-4 py-1 text-[9px] font-black uppercase tracking-widest ${
                        user.status === 'Active' ? 'bg-brand-gold text-brand-matte' : 'bg-brand text-white'
                      }`}>
                        {user.status}
                      </span>
                    </td>
                    <td className="px-10 py-8 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button title="Suspend Operative" className="p-3 text-brand-matte/30 hover:text-brand hover:bg-brand-matte transition">
