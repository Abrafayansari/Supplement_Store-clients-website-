            ...
            <main className="flex-grow p-4 md:p-10 max-w-7xl mx-auto w-full space-y-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-1">
                        <h1 className="text-4xl font-black text-brand-matte tracking-tighter uppercase">Visual <span className="text-brand">Banners</span></h1>
                        <p className="text-brand-matte/40 font-bold uppercase tracking-widest text-[10px]">Manage homepage promotional assets and sales transmission.</p>
                    </div>
                    <button
                    ...
                    >
                        <Plus className="w-5 h-5" /> Deploy New Banner
                    </button>
                </div>
                ...
                {showAddForm && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-brand-matte/80 backdrop-blur-sm">
                        ...
                        >
                            ...
                            <h2 className="text-2xl font-black text-brand-matte uppercase tracking-tighter mb-8">New Banner <span className="text-brand">Configuration</span></h2>
                            <form onSubmit={handleCreate} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-brand-matte/40">Transmission Title (Optional)</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-brand-matte/40">Redirect Protocol (Link)</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-brand-matte/40">Visual Asset (Image)</label>
                                    <div className="relative border-2 border-dashed border-brand-matte/10 p-10 text-center hover:border-brand/30 transition">
                                        ...
                                        <p className="text-[11px] font-black uppercase tracking-widest text-brand-matte/40">
                                            {newBanner.image ? newBanner.image.name : 'Select or drop high-res asset'}
                                        </p>
                                    </div>
                                </div>
                                <button
                                ...
                                >
                                    {uploading ? 'Processing Transmission...' : 'Deploy Asset'}
                                </button>
                            </form>
                        </motion.div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {loading ? (
                        <div className="col-span-full py-20 flex flex-col items-center gap-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-matte/40">Synchronizing Visual Assets...</span>
                        </div>
                    ) : banners.length === 0 ? (
                        <div className="col-span-full py-20 text-center bg-white border border-dashed border-brand-matte/10">
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-matte/40">No active banners in orbit.</span>
                        </div>
                    ) : (
                        banners.map(banner => (
