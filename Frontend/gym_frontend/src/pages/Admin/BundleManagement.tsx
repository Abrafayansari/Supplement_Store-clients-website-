            ...
            <main className="flex-grow p-4 md:p-10 max-w-7xl mx-auto w-full space-y-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-1">
                        <h1 className="text-4xl font-black text-brand-matte tracking-tighter uppercase italic">Bundle <span className="text-brand">Packs</span></h1>
                        <p className="text-brand-matte/40 font-bold uppercase tracking-widest text-[10px]">Create discounted sets of items for your customers.</p>
                    </div>
                    <button
                    ...
                    >
                        <Plus className="w-5 h-5" /> Create New Pack
                    </button>
                </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-brand-warm">
                                    <th className="px-10 py-6 text-[10px] font-black text-brand-matte/40 uppercase tracking-[0.3em]">Bundle Name</th>
                                    <th className="px-10 py-6 text-[10px] font-black text-brand-matte/40 uppercase tracking-[0.3em]">Products</th>
                            ...
                            <tbody className="divide-y divide-brand-matte/5">
                                {loading ? (
                                    <tr>
                                        <td colSpan={4} className="py-20 text-center">
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-matte/40">Loading Bundles...</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="py-20 text-center">
                                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-matte/40">No bundles found.</span>
                                        </td>
                                    </tr>
                                ) : (
                                    filtered.map(bundle => (
                                        <tr key={bundle.id} className="group hover:bg-brand-warm transition">
                                            ...
                                            <td className="px-10 py-8">
                                                <div className="flex flex-col">
                                                    <span className="font-black text-brand text-xl italic tracking-tighter">Rs. {bundle.price}</span>
                                                    {bundle.originalPrice && (
                                                        <span className="text-[10px] text-brand-matte/30 font-bold line-through tracking-widest">Rs. {bundle.originalPrice}</span>
                                                    )}
                                                </div>
                                            </td>
