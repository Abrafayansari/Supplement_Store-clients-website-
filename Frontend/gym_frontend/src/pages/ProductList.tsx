...
//                 {/* ── OUTER: "Categories" — pure state toggle, no Collapsible ── */}
//                 <div className="space-y-6">
//                     <button
...
//                             <button
//                                 onClick={removeCategory}
//                                 className={`flex items-center justify-between w-full text-left text-[11px] font-bold uppercase tracking-widest px-4 py-3 transition-all rounded-sm ${activeCategory === 'All' && activeSubCategory === 'All'
...
//                             {/* ── INNER: Each category — also pure state toggle, no Collapsible ── */}
//                             {categories.map(cat => {
//                                 const isOpen = openCategories.has(cat.name);
//                                 return (
//                                     <div key={cat.name}>
...
//                                                 isOpen ? next.delete(cat.name) : next.add(cat.name);
//                                                 setOpenCategories(next);
//                                             }}
//                                             className={`flex items-center justify-between w-full text-left text-[11px] font-black uppercase tracking-widest px-4 py-2 transition-all border-l-2 ${normalize(activeCategory) === normalize(cat.name)
...
//                                         {/* Subcategories — pure conditional render */}
//                                         {isOpen && (
...
//                                                             setIsFilterOpen(false);
//                                                         }}
//                                                         className={`flex items-center justify-between w-full text-left text-[10px] font-bold uppercase tracking-widest px-4 py-2 transition-all rounded-sm ${normalize(activeSubCategory) === normalize(sub)
...
// ── Moved OUTSIDE ProductList so it never gets recreated on parent re-render ──
interface FilterSidebarProps {
    categories: Category[];
    activeCategory: string;
    activeSubCategory: string;
    openCategories: Set<string>;
    setOpenCategories: (s: Set<string>) => void;
...
const ProductList: React.FC = () => {
    ...
    // shared props for both sidebar instances (desktop + mobile sheet)
                        ...
                        {loading ? (
                            ...
                            <div className="py-32 text-center space-y-8 bg-white/50 border border-brand-matte/5 p-12">
                                ...
                                <div className="space-y-4">
                                    <p className="text-3xl font-black text-brand-matte/20 uppercase tracking-tighter">No Matches Found</p>
                                    <p className="text-[11px] font-bold uppercase tracking-widest text-brand-matte/40 max-w-sm mx-auto">The requested product is not found in the current inventory.</p>
                                    <button onClick={clearAllFilters} className="text-brand font-black uppercase tracking-widest text-[11px] underline underline-offset-4 hover:text-white">
                                        Clear All Filters
                                    </button>
                                </div>
                            </div>
